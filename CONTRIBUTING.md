...
| | |
|---|---|
| `internal/verify/` | 74 Go tests — static checks over the current source. Picked up by `go test ./...`. |
| `web/test/` | 60 test files that bundle the app's TypeScript and run it against a DOM shim, via `npm test` in `web/`. |
