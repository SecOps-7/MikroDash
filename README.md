...
| IP Pools, Address Lists, Interface Lists | Address pools, firewall address-list entries, and interface lists with their members. Removing the address or interface MikroDash is reached through from a list a firewall rule depends on warns first |
...
| Routing Tables, Routing Rules | Routing tables (with the FIB flag) and policy-routing rules in the router's order, with reorder arrows. A rule that could send the router's replies to MikroDash somewhere else, such as `unreachable` on its address or a lookup in a table with no way back, warns first |
...
```bash
docker pull ghcr.io/secops-7/mikrodash:0.8.59
```
