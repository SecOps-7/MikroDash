...
- **Makes changes through the forms' own path**: create, edit or delete one row at a time, with the same permission checks, lockout guards, read-back confirmation, undo history and audit entry (marked as the assistant's). It cannot run arbitrary commands, reboot, or reach another router
