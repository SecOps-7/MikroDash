// The WAN page's "WAN Flow" card: a live Sankey of where the WAN traffic goes.
//
...
// sharing the default route sit inside an ECMP bracket; a standby uplink is a
// dimmed hairline. The geometry is `wan-flow-layout.ts`; this draws it.
//
// NO ROUTER COST. Everything comes from `wan:update`, which the page already
// receives; its rates are borrowed from the interface-status collector, and a
// new reading lands at the WAN poll interval. Between readings the ribbons
// ease toward their new widths and the particles keep flowing.
//
// ── ONE LOOP ────────────────────────────────────────────────────────────────
//
// A single requestAnimationFrame loop (about 30 frames a second) runs while
// the page is open and the data is live, and stops otherwise; a new reading
// restarts it if it had stopped. Under reduced motion there are no particles
// and the ribbons change at once.
...
const live = (): boolean => visible() && socketUp && !isRosDisconnected() && !document.hidden;
...
/** Build the drawing for this set of uplinks at this width. */
function build(host: SVGSVGElement, L: Layout): void {
  host.innerHTML = '';
  host.setAttribute('viewBox', '0 0 ' + L.width + ' ' + L.height);
  host.setAttribute('height', String(L.height));
  ...
  grad('wfTx', 'var(--accent-tx)');
  grad('wfRx', 'var(--accent-rx)');
  const glow = svg('filter', { id: 'wfGlow', x: '-50%', y: '-50%', width: '200%', height: '200%' }, defs);
  ...
  const dots = svg('g', { class: 'wf-dots', filter: 'url(#wfGlow)' }, host);
  const nodes = svg('g', { class: 'wf-nodes' }, host);
  drawn = L.bands.map((b) => {
    const standby = b.active ? '' : ' is-standby';
    const lanes = (): Lanes => ({
      tx: svg('path', { class: 'wf-lane is-tx' + standby, fill: 'url(#wfTx)' }, ribbons),
      rx: svg('path', { class: 'wf-lane is-rx' + standby, fill: 'url(#wfRx)' }, ribbons),
    });
...
/** Write each node's numbers, and the header. Text, not geometry: no easing. */
function writeText(): void {
  const badge = el('wanFlowBadge');
  if (badge) badge.textContent = String(flow.rows.length);
  const totals = el('wanFlowTotals');
  if (totals) {
    totals.innerHTML = flow.rows.length
      ? '<span style="color:var(--accent-rx)">&#8595; ' + esc(fmtMb(flow.rx)) + '</span>' +
        '<span style="color:var(--accent-tx)">&#8593; ' + esc(fmtMb(flow.tx)) + '</span>'
      : '';
  }
  ...
  const empty = el('wanFlowEmpty');
  const why = !flow.rows.length ? 'No uplinks to draw. The table above says why.'
  ...
  el('wanFlowCard')?.classList.toggle('is-paused', !live());
}

/** Lay out and draw for the current reading. */
function draw(): void {
  if (!visible()) return;
  const host = el('wanFlowSvg') as unknown as SVGSVGElement | null;
  const wrap = el('wanFlowWrap');
  if (!host || !wrap) return;
  const width = Math.max(320, wrap.clientWidth || 800);
  target = layout(flow.rows, width);
  const key = flow.rows.map((r) => r.name + (r.active ? '+' : '-')).join('\u0001') + '|' + width;
  if (key !== shapeKey) {
    build(host, target);
    shapeKey = key;
  }
...
function frame(t: number): void {
  if (!live() || !target) { looping = false; el('wanFlowCard')?.classList.add('is-paused'); return; }
  if (t - lastT >= 33) {
...
/** One `wan:update`. */
export function noteWanFlow(p: WANPayload): void {
  flow = flowOf(p.wans);
  ratesAvailable = p.ratesAvailable;
  draw();
}

/** Another router: its uplinks are not this one's. */
export function clearWanFlow(): void {
...
export function initWanFlow(socket: Socket, isVisible: (page: string) => boolean): void {
  visible = () => isVisible('wan');
  document.addEventListener('mikrodash:pagechange', (e) => {
    if ((e as CustomEvent).detail === 'wan') { shapeKey = ''; draw(); }
  });
  socket.on('disconnect', () => { socketUp = false; writeText(); });
  socket.on('connect', () => { socketUp = true; draw(); });
  document.addEventListener('visibilitychange', () => { if (!document.hidden) draw(); });
  // The width decides the layout; a resized window redraws at its new size.
  const wrap = el('wanFlowWrap');
