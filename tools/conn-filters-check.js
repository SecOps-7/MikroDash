...
const { makeTree } = require('./lib/tree-shim.js');
...
if (LIFT.hasReference(ROOT)) {
  ...
  assert.ok(/if\s*\(cc\s*&&\s*_filteredBySrc\)/.test(src),
    'the live applyCountryFilter no longer clears the source filter -- the rule this gate pins ' +
    'has changed upstream, and the port must follow it before this gate is trusted again');
  assert.ok(/if\s*\(ip\s*&&\s*_selectedCC\)/.test(src),
    'the live applySourceFilter no longer clears the country filter -- as above');
}
