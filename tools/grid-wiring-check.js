...
const gridSrc = LIFT.liveSource(ROOT, path.join('public', 'js', 'dashboard-grid.js'));
const appSrc = LIFT.liveSource(ROOT, path.join('public', 'app.js'));
...
if (LIFT.hasReference(ROOT)) {
  assert.ok(appSrc.includes("new CustomEvent('socket:reconnect')"),
    'the live app no longer dispatches socket:reconnect — the grid listener is now dead code');
  for (const sel of ['.dash-drag-handle', '.dash-resize', '.dash-card', '.dash-remove-btn']) {
    assert.ok(gridSrc.includes(sel), 'the live grid no longer uses ' + sel);
  }
}
