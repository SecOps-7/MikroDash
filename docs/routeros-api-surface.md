...
| `/interface/wireguard/add` (listen port) | **A `listen-port` another WireGuard interface already holds makes RouterOS create the interface DISABLED**, with `.about: Listen port already used`; it sends nothing (peer tx 0) and nothing says so unless `.about` is read. Given **no** `listen-port`, RouterOS picks a free one (53513 here) and the interface runs. The bootstrap never names a port: the router only calls out |
| (the tunnel itself, `z8`) | Against `internal/ztp`'s userspace engine, through the CHR's NAT: the handshake completed (the engine saw the router from its translated address), the router's `/tool/fetch` to the server's tunnel address got the reply, the engine saw it from the router's tunnel address, and MikroDash's TCP connects to the API on 8728 and 8729 through the tunnel both succeeded |
| `/ip/service` (several entries) | A router can hold **several entries of one service** (the lab CHR had three `api-ssl`, e.g. per VRF), and `get [find name=api-ssl] address` then fails the whole import: `invalid internal item number (/ip/service/get; line 20)`. The bootstrap walks `[find where …]` ids one at a time |
| (a generated bootstrap, `z10`) | `ztp.RemoteScript`, `/import`ed on the CHR: the import returned `ret=true`; the router enrolled through the tunnel with its identity, model, version, system ID, the token and a 32-character password it made itself; MikroDash then **signed in through the tunnel on 8728 and on 8729 (TLS)** with that password; the enrolment script and its scheduler removed themselves; and the same login from outside the tunnel was refused (`not allowed to login from this address`) |
| `:serialize to=json` | `[:serialize to=json value={"token"="t0k";"serial"=$id;"n"=3}]` gives `{"n":3,"serial":"AB12\"x","token":"t0k"}`: an associative array becomes correct JSON, quotes in values escaped, so the enrolment body is built by RouterOS rather than by string concatenation |
| `/system/script/add` + scheduler | A stored multi-line script run by a scheduler **by name** (`on-event=<script>`) ran, then removed its scheduler and **itself**, both gone within 12 s |
| (a quoted `source=`) | A line `/system script add name=… source=<cfgtpl.QuoteROS(text)>` stores the text **exactly** (`\0A` becomes a newline, `\$` and `\"` their characters, `\5B`/`\5D` the brackets) and running it gives the expected output. The generator embeds the enrolment script this way |
