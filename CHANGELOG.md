...
- **The AI Agent reaches every new page**, can ping and traceroute from the router, and
  ...
  first; reboots need the router's name typed back.
- **The AI Agent sees more live data**: WAN status, packages, router users, queues, logs,
...
- New pages are declared once in `internal/areas` and generated by `cmd/areagen`, served
  by one collector.
- `docs/mikromcp-parity.md` maps every MikroMCP tool, held to the code by a test.
- Raw RouterOS command tools exist behind a global-admin gate and a setting that is off by
