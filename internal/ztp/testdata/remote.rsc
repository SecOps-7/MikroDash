# MikroDash zero-touch provisioning: Branch 12 (remote, through the tunnel)
# Instance inst-0123. Valid until 2026-09-29 12:00 UTC.
# THIS FILE HOLDS A SECRET (this device's tunnel key and a one-time token). Keep it like a password.
# Run it on the router: upload the file and import it, or paste it into a terminal.
# It is safe to run twice.

/interface wireguard
:if ([:len [find name="mikrodash-ztp"]] = 0) do={ add name="mikrodash-ztp" private-key="BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBE=" comment="MikroDash ZTP" }
...
/ip address
:if ([:len [find interface="mikrodash-ztp"]] = 0) do={ add address="10.249.1.7/32" network="10.249.0.1" interface="mikrodash-ztp" comment="MikroDash ZTP" }
/ip firewall filter
