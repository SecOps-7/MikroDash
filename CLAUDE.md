...
- **Declared by a resource**, and run by the generic resource write path (`verdictFor` in
  ...
  `selfPath`, `fwGuard`, `wifiInherit`, `capsmanPush`, `routePath`, `addressPath`, `queueThrottle`, `selfAccount`, `listLockout`, `serviceLockout`, `certLockout`, `codeGate`, `rulePath`, `ipsecPath`, `tunnelDefault`. **A resource declaring any other guard has
